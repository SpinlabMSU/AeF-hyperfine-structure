// ZeemanMain.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
/*
...
#define USE_TRIVIAL
#if defined(USE_EXPECTATION_VALUES)
...
#define USE_DEVONSHIRE
    ...
    cxxopts::Options options("aef-zeeman-main", "Program to calculate the hyperfine structure of"
        " diatomic Alkaline-monofluoride molecules under the influence of a magnetic field");
    options.add_options()
        ("h,help", "Print usage")
        ("e,E_z", "Externally-applied Electric field along z axis", cxxopts::value<double>())
        ("b,B_max", "Maximum magnetic field [mG]", cxxopts::value<double>())
        ("B,B_min", "Minimum magnetic field [mG]", cxxopts::value<double>())
        ...
        ("t,zeeman_iterations", "Number of iterations to perform the zeeman loop for", cxxopts::value<size_t>())
    ...
    if (result.count("zeeman_iterations")) {
        nZeemanIterations = result["zeeman_iterations"].as<size_t>();
    }

    if (result.count("B_max")) {
        max_B_z_G = result["B_max"].as<double>();
    }

    if (result.count("B_min")) {
        min_B_z_G = result["B_min"].as<double>();
    }
    ...
    // info lines
    {
        ...
        std::ofstream oZee(dpath / "ZEEMAN.log", std::ios::trunc | std::ios::out);
        oZee << fmt::format("Zeeman {}\t{}\t{}\t", min_B_z_G, max_B_z_G, nZeemanIterations, E_z);
        oZee.close();
    }
    ...
    // create output file
    auto fpath = dpath / "zeeman_shift_gnd.csv";
    std::ofstream oStk(fpath, std::ios::trunc | std::ios::out);

    auto epath = dpath / "zeeman_spectrum.csv";
...
#define EVAL(val) val
    ...
    Eigen::MatrixXcd lowest_energies(nZeemanIterations, nLowestStates);
    ...
    // directory to put molecular state info, unfortunately called devonshire info
    auto devpath = dpath / "devonshire_info";
    fs::create_directories(devpath);

    // Stark loop
    prev_time = log_time_at_point("About to start Zeeman loop", start_time, prev_time);
    ...
    const double scale_Bz_G = max_B_z_G - min_B_z_G;
    const double scale_Bz_T = scale_Bz_G * unit_conversion::Tesla_per_Gauss;
    const double offset_Bz_T = min_B_z_G * unit_conversion::Tesla_per_Gauss;

    const double Ez_mhz = E_z * unit_conversion::MHz_D_per_V_cm;

    // note: we now need to go backwards to track from calc_E_z
    for (int fdx = nZeemanIterations - 1; fdx >= 0; fdx--) {
        double field_divisor = nZeemanIterations - 1.0;
        const double B_z_T = (scale_Bz_T) * (fdx / field_divisor) + offset_Bz_T;
        const double B_z_G = B_z_T * unit_conversion::Gauss_per_Tesla;
        ...
        std::cout << fmt::format("Z-axis Magnetic field strength is {} G", B_z_G) << std::endl;
        ...
        if (round_files) {
            dev_out_fname = fmt::format("info_Bz_{}.csv", std::lround(1000 * B_z_G));
        } else {
            dev_out_fname = fmt::format("info_Bz_{:.1f}.csv", 1000*B_z_G);
        }
    ...
    for (int fdx = 0; fdx < nZeemanIterations; fdx++) {
        const char* sep = "";
        for (int sdx = 0; sdx < nLowestStates; sdx++) {
    ...
    std::cout << "--------- Zeeman loop completed ---------" << std::endl;
    prev_time = log_time_at_point("Completed Zeeman loop", start_time, prev_time);
