...
df = run.parse_gnd_stark_shift()

prod_coeffs = False
if run.coeff_set != None:
    prod_coeffs = run.coeff_set.lower().startswith("prod")
...
gca.tick_params(axis='both', which='both', direction='inout')
if new_gas_plots:
    textstr = '$F_1=1$ Stark shift below $F_1=0$'
else:
    textstr = f'$F_1=1$ Stark shift above ground'
if not doe_plot or not prod_coeffs:
    gca.text(0.05, 0.1, textstr, transform=gca.transAxes, fontsize=14, verticalalignment='top', bbox=props)
else:
    gca.text(0.05, 0.9, textstr, transform=gca.transAxes, fontsize=14, verticalalignment='top', bbox=props)
if not new_gas_plots:
...
#
if not doe_plot:
    fig.suptitle(f"N=0, F$_1$=0,1 Stark shift for run {run.run}", y=0.999)
else:
    s1 = ""
    if not prod_coeffs:
        s1 = ", reproducing heavy-zero"
    s = f"New code gas phase heavy-half N=0, F$_1$=0,1 Stark shift{s1}"
    fig.suptitle(s, y=0.999)
