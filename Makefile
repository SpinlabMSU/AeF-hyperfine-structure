...
ROOT_CFLAGS:=$(shell root-config --cflags | sed -r 's/-std=(\w|\+)+ //g') # note: want a more recent C++ version than root requires
ROOT_LDFLAGS:=$(shell root-config --ldflags)
ROOT_LDLIBS:=$(shell root-config --glibs)
CXX_VSN:=-std=gnu++23 -fmodules-ts -fopenmp
CXXFLAGS:=$(INCLUDES) $(CXX_VSN) -O4 -fPIC -flto $(ROOT_CFLAGS) -g -march=native -D_GNU_SOURCE
LDFLAGS=-L. -pthread -fopenmp -flto $(ROOT_LDFLAGS) -static-libstdc++ -static-libgcc -g -march=native -O4
CUDA_LIBS:=-lcusolver -lcublas -lcublasLt -lcuda -lcudart_static
LDLIBS:=-l:./libSpinlabHyperfine.a -lgsl -lgslcblas $(CUDA_LIBS) -lz -lm $(ROOT_LDLIBS)
...
operatorVisualizer perturbation_analyzer aef_main nodev_aef_mn deven_aef_mn
...
$(patsubst %.cpp,%.o,$(wildcard SpinlabHyperfineLib/src/systems/*.cpp))

libSpinlabHyperfine.a: $(LSPHF_OBJ)
	$(AR) rcs $@ $^
libSpinlabHyperfine.so: libSpinlabHyperfine.a $(LSPHF_OBJ)
	$(CXX) -o $@ -shared $(LDFLAGS) $(CXXFLAGS) $^ $(LDLIBS)
...
AeF_main/AeF-main.inl:
	(cd AeF_main && ./prebuild.sh)
AeF_main/AeF-main.o: AeF_main/AeF-main.inl AeF_main/AeF-main.cpp AeF_main/AeF-main.h
aef_main: AeF_main/AeF-main.o libSpinlabHyperfine.so
	$(CXX) -o $@ $(CXXFLAGS) $(LDFLAGS) $< $(LDLIBS)

deven_aef_mn: AeF_main/AeF-main.cpp libSpinlabHyperfine.so AeF_main/AeF-main.h
	$(CXX) -o $@ $(CXXFLAGS) -D_MAKEFILE_PROVIDES_DEVFLAG -DUSE_DEVONSHIRE $(LDFLAGS) $< $(LDLIBS)

nodev_aef_mn: AeF_main/AeF-main.cpp libSpinlabHyperfine.so AeF_main/AeF-main.h
